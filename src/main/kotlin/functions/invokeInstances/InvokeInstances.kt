package functions.invokeInstances

class Request(val method: String, val query: String, val contentType: String)
class Response(var contents: String, var status: Status) {
    // Function extensions provides as dsl for writing codes
    operator fun invoke(status: Status.() -> Unit) {
    }
}

class Status(var code: Int, var description: String)

class RouteHandler(val request: Request, val response: Response) {
    var executeNext = false
    fun next() {
        executeNext = true
    }
}

// Takes extension function
fun routeHandler(path: String, f: RouteHandler.() -> Unit): RouteHandler.() -> Unit = f

fun main(args: Array<String>) {

    // access the request and response. Because they are properties of RouteHandler
    // make dsl like route handler
    routeHandler("index.html") {
        if (request.query.isNotEmpty()) {
            //process
        }
        // make this dsl.
        response {
            code = 404
            description = "NotFound"
        }
    }

    val manager = Manager()

    // Kotlin helps invoke some functionality on that class by simply using instance of class and passing any values as parameters
    manager("Do something")
    manager(12345)
}

class Manager {
    operator fun invoke(value: String) {
        println(value)
    }

    operator fun invoke(value: Int) {
        println(value)
    }
}

